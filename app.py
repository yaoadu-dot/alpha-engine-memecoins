...
with st.sidebar:
    ...
    max_age = st.slider("Max Age (Hours)", 1, 24, 6)
...
def calculate_rug_score(liq, mcap):
    """Simple heuristic: Ratio of Liquidity to Market Cap."""
    if mcap <= 0: return -5
    ratio = liq / mcap
    if ratio > 0.1: return 3   # Good liquidity depth
    if ratio > 0.01: return 0  # Moderate
    return -5                  # Likely low-liquidity rug
...
def calculate_deltas(pools):
    ...
    for p in pools:
        ...
        # Keep window clean
        history = st.session_state["snapshots"][pid] = [s for s in st.session_state["snapshots"][pid] if (current_time - s) <= 120]
        ...
        momentum_score = 3 if vol_1m > (liq * 0.01) else 0 # High vol vs liq
...
if raw_pools:
    ...
    if not df.empty:
        ...
        st.markdown("### 🎯 Verified Momentum Targets")
        for _, row in df_filtered.iterrows():
            st.success(f"**{row['Asset']}** | Age: {row['Age (h)']}h")
            st.code(row['Pool ID'])
    else:
        st.warning("No coins met the safety/momentum criteria.")
else:
